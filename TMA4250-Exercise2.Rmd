...
<<<<<<< HEAD
The Neyman-Scott cluster model (mother-chid model) for event spatial variables is given below. The mother model is usually a stationary Possion RF with intensity $\lambda_M$. Centered at every mother location, $\boldsymbol{x}^M_j$, a set of child events are independently located, with event count given by the pdf $p(k^c)$ and intensity pdf $p(\boldsymbol{x}|\boldsymbol{x}^M_j)$. Censoring is done in order to make sure that the child-event is in D. The model can be expressed in terms of the event locations as $\mathbb{X}^{NS}_D = (\boldsymbol{x}_1, \boldsymbol{x}_2, \dots, \boldsymbol{x}_{k_D})$. The locations can also be expressed $\boldsymbol{x}_{ji}$ with $j = 1, \dots, k_D^M$, and $i = 1, \dots, k_{D_j}^c$ where $k_D = \sum_j k^c_{D_j}$, i. e. the i'th child of mother j. We then obtain 

$$
\mathbb{X}_D^{NS} \sim p(\boldsymbol{x}) = \prod_{j = 1}^{k^M_D} \bigg[ \prod_{i = 1}^{k^c_{D_j}} \frac{I(\boldsymbol{x}_{ji} \in D) p (\boldsymbol{x}_{ji}|\boldsymbol{x}^M_j)} {P(D | \boldsymbol{x}^M_j)} \bigg] \times p(k^c_{D_j} | k^c_j, \boldsymbol{x}^M_j) p(k^c_j) \times \frac{1}{|D|^{k^M_D}}p(k^M_D) \\
= \prod_{j = 1}^{k^M_D} \bigg[ \prod_{i = 1}^{k^c_{D_j}} \frac{I(\boldsymbol{x}_{ji} \in D) \phi_3(\boldsymbol{x}_{ji}; \boldsymbol{x}^M_j, \sigma^2_c I_3) } {\Phi_3(D | \boldsymbol{x}^M_j, \sigma^2_c I_3)} \bigg] \times {k^c_l \choose k^c_{D_j}} \Phi_3(D | \boldsymbol{x}^M_j, \sigma^2_c I_3) \Big( 1 - \Phi_3(D | \boldsymbol{x}^M_j, \sigma^2_c I_3) \Big)^{k^c_j - k^c_{D_j}} p(k^c_j) 
\\ \times \frac{{\lambda_M}^{k^M_D}}{k^M_D!} \exp(-\lambda_M |D|)
$$

Where $k^j_{D_j} \leq k^c_j}$ The latter equality corresponds to the case of a mother-child model with a stationary Poission RF as mother model, having intensity $\lambda_M$, and a Gaussian intensity child-model with spread $\sigma^2_c$ where the child set size is given by $p(k^c_j)$. The parameters of this model is $\boldsymbol{\theta}_{pNS} = (\lambda_M, \sigma^2_c, p(.))$.
...
=======
The Neuman-Scott cluster model (mother-chid model) for event spatial variables is given below. The mother model is usually a stationary Possion RF with intensity $\lambda_M$. Centered at every mother location, $\mathbf{x}^M_j$, a set of child events are independently located, with event count given by the pdf $p(k^c)$ and intensity pdf $p(\mathbf{x}|\mathbf{x}^M_j)$. Censoring is done in order to make sure that the child-event is in $D$. The model can be expressed in terms of the event locations as $\mathbb{X}^{NS}_D = (\mathbf{x}_1, \mathbf{x}_2, \dots, \mathbf{x}_{k_D})$. The locations can also be expressed $\mathbf{x}_{ji}$ with $j = 1, \dots, k_D^M$, and $i = 1, \dots, k_{D_j}^c$ where $k_D = \sum_j k^c_{D_j}$, i. e. the i'th child of mother j. We then obtain 
\begin{align*}
\mathbb{X}_D^{NS} &\sim p(\mathbf{x}_{ji};j=1,\dots,k_D^M; i=1,\dots,k_{D_j}^c)\\
&= \prod_{j = 1}^{k^M_D} \bigg[ \prod_{i = 1}^{k^c_{D_j}} \frac{I(\mathbf{x}_{ji} \in D) p (\mathbf{x}_{ji}|\mathbf{x}^M_j)} {P(D | \mathbf{x}^M_j)} \bigg] \times p(k^c_{D_j} | k^c_j, \mathbf{x}^M_j) p(k^c_j) \times \frac{1}{|D|^{k^M_D}}p(k^M_D) \\
&= \prod_{j = 1}^{k^M_D} \bigg[ \prod_{i = 1}^{k^c_{D_j}} \frac{I(\mathbf{x}_{ji} \in D) \phi_2(\mathbf{x}_{ji}; \mathbf{x}^M_j, \sigma^2_c I_2) } {\Phi_2(D | \mathbf{x}^M_j, \sigma^2_c I_2)} \bigg]\\
&\times {k^c_l \choose k^c_{D_j}} \Phi_2(D | \mathbf{x}^M_j, \sigma^2_c I_2) \Big( 1 - \Phi_2(D | \mathbf{x}^M_j, \sigma^2_c I_2) \Big)^{k^c_j - k^c_{D_j}} p(k^c_j) 
\\
&\times \frac{{\lambda_M}^{k^M_D}}{k^M_D!} \exp(-\lambda_M |D|),
\end{align*}
...
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
...
<<<<<<< HEAD
To get an empirical estimate of the model parameters, we study the redwood tree dataset. There seems to be approximatly $10$ clusters, and a total of $62$ observations. Hence a mother intensity of $10$ and a child intensity of $6$ might be fitting. The spread in the clusters corresponds to a $\sigma^2_c$ of approximatly $1.5 \cdot 10^{-3}$, which we have obtained by trial and error. Below the redwood tree data is compared with one realization of this model. 
```{r, echo = F, eval = T, out.width = "33%"}
#set.seed(4250)
=======
...
sim_mat = simulate_neuman_scott(10, 6, 1.5e-3)
=======
...
<<<<<<< HEAD
  sim_mat = simulate_neuman_scott(10, 5, 1.5e-3)
=======
  sim_mat = simulate_neuman_scott(8, 7.75, 2.5e-3)
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
...
```
<<<<<<< HEAD
We note that the $L$-function is just outside our confidence interval for some small values of $t$ for these parameters. But if we choose $\lambda_M$ larger and $\lambda_c$ smaller, we can get it completly contained in the confidence interval. $\lambda_M = 15$ and $\lambda_c = 3$ is plotted below. 
...
for (i in 1:100){
  ...
  # simulate with our parameters
  sim_mat = simulate_neuman_scott(15, 3, 1.5e-3)
  df_redwood = data.frame(x = sim_mat[,1], y = sim_mat[,2])
...
ggplot() +
    ...
    geom_line(data = df_redwood, aes(x = x, y = pred), size = 1, col = 'red') + 
    geom_line(data = df_redwood, aes(x = x, y = lb), size = 1, col = 'blue', linetype = "dashed") + 
    geom_line(data = df_redwood, aes(x = x, y = ub), size = 1, col = 'blue', linetype = "dashed") + ylab("L(t)") + xlab("t") + ggtitle("Empirical Monte Carlo test for our choice of model parameters")
```
=======
...
$$
<<<<<<< HEAD
\mathbb{X}^S_D | (k_D = k) \sim p(\boldsymbol{x} | k_D = k) \propto \prod_{i, j \in \{1, \dots, k\}} \exp(-\phi(\boldsymbol{\tau}_{ij}))
$$
with $\boldsymbol{\tau}_{ij} := \boldsymbol{x}_i - \boldsymbol{x}_j$ and where $\phi : \mathbb{R}^n \to \mathbb{R}$ is decreasing with increasing $\| \boldsymbol{\tau}_{ij} \|$. $\phi(.)$ is possibly anisotropic. A typical form of $\phi(.)$ is 
=======
\big[\mathbb{X}^S_D | (k_D = k)\big] \sim p(\mathbf{x}_1,\dots,\mathbf{x}_k | k_D = k) \propto \prod_{i, j \in \{1, \dots, k\}} \exp(-\phi(\boldsymbol{\tau}_{ij})),
$$
with $\boldsymbol{\tau}_{ij} := \mathbf{x}_i - \mathbf{x}_j$ and $\phi(\boldsymbol{\tau}_{ij}) : \mathbb{R}^n \to \mathbb{R}_\oplus$ is decreasing with increasing $| \boldsymbol{\tau}_{ij} |$. $\phi(.)$ is possibly anisotropic with $\tau_{ij} = |\boldsymbol{\tau}_{ij}|\in\mathbb{R}_\oplus$ (BETYR DETTE AT DET ER ULIKE BIDRAG FOR FORKJELL I KOMPONENTENE?). A typical form of $\phi(.)$ is 
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
$$
\phi(\tau) = 
\begin{cases}
  \phi_0, \ &\tau  \in [0, \tau_0], \\
  \phi_0 \exp(-\phi_1 (\tau - \tau_0)), \ &\tau \in [\tau_0,\infty),
\end{cases}
$$
<<<<<<< HEAD
with model parameters $\boldsymbol{\theta}_{pS|k} = (\tau_0, \phi_0, \phi_1)$ all non-negative. 
...
=======
with model parameters $\boldsymbol{\theta}_{pS|k} = (\tau_0, \phi_0, \phi_1)$ all positive ($\tau_0$ can be zero). WHAT IS WHAT p. 128. This gives a repulsive model, as it enforces penalites (lower probability) whenever events are located close to eachother.

This Strauss event RF model is not analytically tractable, but we can simulate from it using the algorithm below.
```{r, echo = T, eval = T}
simulate_strauss = function(k, tau0, phi0, phi1, iterations = 1000) {
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
...
<<<<<<< HEAD
  result = matrix(0.5, nrow = k, ncol = 2)
  #result = matrix(pmax(1e-3, pmin(0.99, rnorm(n = 2*k, mean = rep(0.5, times = 2*k, sd = 2)))), nrow = k, ncol = 2)
=======
  result = matrix(runif(k), nrow = k, ncol = 2)
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
  
  # the algorithm is asymptotically correct. Increase iterations to imporove result
...
<<<<<<< HEAD
To get an empirical estimate of the model parameters, we study the biological cells dataset. There is a total of $42$ observations. Hence we set $k$ equal to $42$. To obtain estimates of $\tau_0, \phi_0$ and $\phi_1$ one approach could be to fix $\tau_0$ and then find the ML estimates of $\phi_0$ and $\phi_1$. $\tau_0 = 0.1$ and $\phi_0, \phi_1$ large would seem fitting - But the algorithm seems to converge slow for these values. So to get the simulations done in reasonable time we set $\tau_0 = 10^{-5}$, $\phi_0 = 10^{-3}$ and $\phi_1 = 100$. These parameters does not fit the data well. Iterating $1000$ times seems to work for these values. Below the biological cells data is compared with one realization of this model. 
```{r, echo = F, eval = T, out.width = "33%"}
=======
...
plot(data_cells, main = "Cells data", xlab = "x", ylab = "y")

# display the simulated data
<<<<<<< HEAD
sim_mat = simulate_strauss(42, 1e-5, 1e-3, 100) # tau0, phi0, phi1
...
simulation = simulate_strauss(42, 0.4, 0.5, 0.5)
plot(simulation, main = "Strauss repulsion model realization", xlab = "x", ylab = "y")
```

<<<<<<< HEAD
We now want to justify study our choice of parameters using an MCMC test on the $L$-function. We have done this below.
```{r, echo = F, eval = T, out.width = "33%"}
k_cells <- length(data_cells$y)
=======
We now want to justify our choice of parameters using an MCMC test on the $L$-function. We have done this below.
```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:StraussMonteCarlo}Empirical monte carlo test of our model parameters. We have computed the L-functions from 100 realizations of the Strauss repulsion model, and display these along with the 90$\\%$ interval and the L-function of the Celss data."}
>>>>>>> 4dc420fb79a45fff8d1bdf018a3a368d08d5f741
L_matrix_cells <- matrix(NA, nrow = 100, ncol = length(cells_L$x))
ppinit("cells.dat")
...
<<<<<<< HEAD
  ...
  ppregion()
=======
  sim_mat = simulate_strauss(42, 0.4, 0.5, 0.5)
  df_cells = data.frame(x = sim_mat[,1], y = sim_mat[,2])
...
ggplot() +
    theme_bw() +
    theme(panel.grid=element_blank()) +
    geom_line(data = mdat_cells, aes(x=x, y=value, group=trial, col = "Realizations"), size=0.2, alpha=0.1) +
    geom_line(data = df_cells, aes(x = x, y = pred, col = "Cells data"), size = 1) + 
    geom_line(data = df_cells, aes(x = x, y = lb, col = "5th percentile"), size = 1, linetype = "dashed") + 
    geom_line(data = df_cells, aes(x = x, y = ub, col = "90th percentile"), size = 1, linetype = "dashed") + ylab("L(t)") + xlab("t") + ggtitle("Empirical Monte Carlo test for our choice of model parameters") + scale_color_manual(values = c("blue", "blue", "red", "grey"))
```
The plot clearly shows that this choice of parameter values does not fit the data. Moreover, and a bit concerning, is that the $L$-function of the simulations lies above the line $L(t) = t$, which we expect for a clustering model, not a repulsive model. 
