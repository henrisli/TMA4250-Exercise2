...
sim_mat = simulate_neuman_scott(10, 6.2, 1.5e-3)

plot(sim_mat, main = "Neumann-scott realization", xlab = "x", ylab = "y")
```
The two plots look similar, but we want to justify our choice of parameters using an MCMC test on the $L$-function.
```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:NSMonteCarlo}Empirical monte carlo test of our model parameters with mother intesity of 10 and child intensity of 6.2. We have computed the L-functions from 100 realizations of the Neumann-Scott model, and display these along with the 90$\\%$ interval and the L-function of the Redwood data."}
set.seed(425)
k_redwood <- length(data_redwood$y)
L_matrix_redwood <- matrix(NA, nrow = 100, ncol = length(redwood_L$x))
ppinit("redwood.dat")
...
```
We note that the $L$-function is just outside our confidence interval for some small values of $t$ for these parameters. But if we choose $\lambda_M$ larger and $\lambda_c$ smaller, we can get it completly contained in the confidence interval. The Empirical Montecarlo test for the Neumann-Scott model for $\lambda_M = 15$ and $\lambda_c = 3$ is plotted in figure \ref{fig:NS2MonteCarlo}. 

```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:NS2MonteCarlo}Empirical monte carlo test of our model parameters with mother intensity of 15 and child intensity of 3. We have computed the L-functions from 100 realizations of the Neumann-Scott model, and display these along with the 90$\\%$ interval and the L-function of the Redwood data."}
...
ggplot() +
    ...
    geom_line(data = df_redwood, aes(x = x, y = pred, col = "Redwood data"), size = 1) + 
    geom_line(data = df_redwood, aes(x = x, y = lb, col = "5th percentile"), size = 1, linetype = "dashed") + 
    geom_line(data = df_redwood, aes(x = x, y = ub, col = "95th percentile"), size = 1, linetype = "dashed") + ylab("L(t)") + xlab("t") + ggtitle("Empirical Monte Carlo test for our 2nd choice of model parameters") + scale_color_manual(values = c("blue", "blue", "grey", "red"))
```
We can then display one realization and compare it to the Redwood data. This is done below.
...
plot(sim_mat, main = "Neumann-scott realization", xlab = "x", ylab = "y")
```
From the displayed realization it is hard to tell if the parameters $\lambda_M=15,\lambda_c=3$ fit the Redwood data better, even though the $L$-function of the Redwood data was contained in the 0.9-interval for this choice of parameters.
...
$$
with $\boldsymbol{\tau}_{ij} := \mathbf{x}_i - \mathbf{x}_j$ and $\phi(\boldsymbol{\tau}_{ij}) : \mathbb{R}^n \to \mathbb{R}_\oplus$ is decreasing with increasing $| \boldsymbol{\tau}_{ij} |$. $\phi(.)$ is possibly anisotropic with $\tau_{ij} = |\boldsymbol{\tau}_{ij}|\in\mathbb{R}_\oplus$. A typical form of $\phi(.)$ is 
$$
\phi(\tau) = 
\begin{cases}
  \phi_0, \ &\tau  \in [0, \tau_0], \\
...
\end{cases}
$$
with model parameters $\boldsymbol{\theta}_{pS|k} = (\tau_0, \phi_0, \phi_1)$ all positive ($\tau_0$ can be zero). This gives a repulsive model, as it enforces penalites (lower probability) whenever events are located close to eachother.
...
simulate_strauss = function(k, tau0, phi0, phi1, iterations = 1000) {
  ...
  # we have chosen to initialize the points uniformly on D
...
plot(sim_mat, main = "Strauss repulsion model realization", xlab = "x", ylab = "y")
```

We now want to study our choice of parameters using an MCMC test on the $L$-function. We have done this below.

```{r, echo = F, eval = T, out.width = "50%", fig.align = "center", fig.cap = "\\label{fig:StraussMonteCarlo}Empirical monte carlo test of our model parameters. We have computed the L-functions from 100 realizations of the Strauss repulsion model, and display these along with the 90$\\%$ interval and the L-function of the Cells data."}
L_matrix_cells <- matrix(NA, nrow = 100, ncol = length(cells_L$x))
ppinit("cells.dat")
...
```
The plot in figure \ref{fig:StraussMonteCarlo} clearly shows that this choice of parameter values does not fit the data. Moreover, and a bit concerning, is that the $L$-function of the simulations lies above the line $L(t) = t$, which we expect for a clustering model, not a repulsive model. 
